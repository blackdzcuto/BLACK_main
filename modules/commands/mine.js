    ...
    case "shop":
      switch (a.body) {
        ...
        case "2": {
          ...
          return e.sendMessage({
            body: "[ = [ Shop Pickaxe] = ]\n===DANH SÁCH CÚP===\n\n" + h.join("\n") + "\nReply Tin Nhắn Bot Và Chọn Theo S��� Thứ Tự",
            attachment: g
          }, a.threadID, ((e, n) => {
            global.client.handleReply.push({
              name: this.config.name,
        ...
        case "4":
          if (i < 1e3) e.sendMessage("[ HỆ THỐNG MINE ] - Bạn không đủ tiền để nâng cấp", a.threadID);
          else {
        ...
        case "7": {
          ...
          if ("chuadangky" == s.data.msg) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chưa đăng ký hệ thống", a.threadID); {
            const r = s.data.damage,
              ...
              c = s.data.msg;
            s.data.images;
            if ("die" == s.data.msg) return i < 2000 ? e.sendMessage("[ HỆ THỐNG DUNGEON ] - Số Dư Của Bạn Không Đủ Đề Hồi Sinh ! Vui lòng kiếm đủ 2000$ để hồi sinh", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage("[HỆ THỐNG DUNGEON] - Bạn đã chết ! Hệ Thống Sẽ Tự Động Hồi Sinh Với Cái Gía Là 2000$", a.threadID));
    ...
    case "repair":
      if (a.body == a.body) {
        if (i < 2000) return e.sendMessage("[ HỆ THỐNG MINE ]Bạn chưa đủ tiền để nâng cấp\nSố tiền : 2000$", a.threadID); {
          ...
          "1" == a.body && ("false" == n.data.msg ? e.sendMessage("[ HỆ THỐNG MINE ] -Bạn chưa mua cúp ! Không thể sửa chữa", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage(`[ HỆ THỐNG MINE ] - ${n.data.msg}`, a.threadID))), "2" == a.body && ("false" == n.data.msg ? e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chưa mua kiếm ! Không thể sửa chữa", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage(`[ HỆ THỐNG MINE ] - ${n.data.msg}`, a.threadID)))
        }
      }
      break;
    case "weapons":
